...
func buildObstacles(level int) map[point]bool {
	...
	count := 3 + level
	for i := 0; i < count; i++ {
		x := 6 + (i*7)%(gridWidth-12)
		y := 5 + (i*5)%(gridHeight-10)
		if nearStart(point{x: x, y: y}) {
			continue
		}
		obstacles[point{x: x, y: y}] = true
		obstacles[point{x: x + 1, y: y}] = true
	}
	return obstacles
}
...
func (g *Game) rememberKeys() {
	for _, key := range []ebiten.Key{ebiten.KeyP, ebiten.KeyEscape, ebiten.KeyR} {
...
func (g *Game) spawnApple() {
	open := make([]point, 0, gridWidth*gridHeight)
	for y := 0; y < gridHeight; y++ {
		for x := 0; x < gridWidth; x++ {
			p := point{x: x, y: y}
			if g.occupied(p) || g.hitsObstacle(p) {
				continue
			}
			open = append(open, p)
		}
